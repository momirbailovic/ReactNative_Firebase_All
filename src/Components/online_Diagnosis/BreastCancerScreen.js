...
import { Content, Button, Text, Container, Icon, Left, Body, Right } from "native-base";
import QuestionDispayCard from "./questionDispayCard";
import BreastCancerQue from "../Diagnosis_Content/BreastCancerQue";
...
export default class BreastCancerScreen extends React.Component {
    ...
    submit(goBack=false) {
        if ((2 * this.questionCard.length) === this.state.response.length) {
            ...
            this.props.submit("BreastCancer", res, true, false, goBack)
        }
        else {
            ...
            Alert.alert(
                ...
                [
                    ...
                    {
                        text: 'Continue later',
                        onPress: () => this.props.submit("BreastCancer", this.state.response, false, false, goBack),
                    },
                ],
                { cancelable: true }
            )
    ...
    save() {
        res = this.state.response;
        this.props.submit("BreastCancer", res, false, true)
    }
    ...
    Que() {
        //for first rendering
        if (!this.state.response.length || this.state.incompleteForm) {
            this.questionCard = [];
            let dataa = BreastCancerQue();
            let j = 0;
            for (let i = 1; i <= dataa.numberOfQuestion; i++) {
                ...
                let tempOption = [];
                tempOption = dataa.Ques[i].option
    ...
    render() {
        if (this.props.gender === "female") {
            ...
            this.props.submit("BreastCancer", null, true)
            return null
        }
    }
}
